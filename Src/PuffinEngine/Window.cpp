...
Window::Window() {
    ...
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, Configuration::instance().getOpenGLVersion().first);
    glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, Configuration::instance().getOpenGLVersion().second);
    glfwWindowHint(GLFW_SAMPLES, Configuration::instance().getMsaaSamples());
    glfwWindowHint(GLFW_RESIZABLE, GL_FALSE);

    GLint count = 0;
    auto monitors = glfwGetMonitors(&count);
    auto target_monitor = monitors[Configuration::instance().getTargetMonitorIndex()];
    ...
    handle_ = glfwCreateWindow(wnd_width, wnd_height, caption_.c_str(),
        Configuration::instance().isFullscreenEnabled() ? target_monitor : nullptr, nullptr);
    if (!handle_) {
